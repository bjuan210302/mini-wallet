...
function generateAccountSwitchButtons() {
    const buttonsContainer = document.querySelector('#accounts-container')
    accounts.forEach(acc => {
        const accButton = document.createElement('button')
        accButton.setAttribute('acc-id', acc.name)
        accButton.className =
            'bg-neutral-800 px-8 py-6 rounded-md border border-neutral-800'
            + ' hover:bg-neutral-700 hover:drop-shadow-[0_35px_35px_rgba(100,100,100,0.2)] transition-all'
        accButton.innerText = acc.name
        accButton.onclick = () => {
            selectedAccount.next(acc.name)
        }
        buttonsContainer.appendChild(accButton)
    })
}
...
function doSubscritions() {
    ...
    txsObservable.subscribe((tx) => {
        ...
        if (!Array.isArray(tx)) {
            const txRow = createRowForTx(tx, true)
    ...
    selectedAccount.subscribe(accName => {
        // Put border to button and remove border from other buttons
        const selectedButtonAccount = document.querySelector(`[acc-id="${accName}"`)
        selectedButtonAccount.classList.remove('border-neutral-800')
        const buttonsContainer = document.querySelector('#accounts-container')
        buttonsContainer.childNodes.forEach(c => {
            if (c.getAttribute('acc-id') === accName)
                return
            c.classList.add('border-neutral-800')
        })
